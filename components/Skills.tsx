          ...
          {SKILLS.map((category, index) => {
            ...
            return (
              <RevealOnScroll key={index} delay={index * 100} direction="bottom">
                <div 
                  className="group relative h-full bg-white dark:bg-[#0a0f1c] border border-slate-200 dark:border-slate-800 rounded-3xl p-8 overflow-hidden hover:border-slate-300 dark:hover:border-slate-600 transition-colors duration-500 shadow-xl dark:shadow-none"
                >
                  {/* Spotlight Effect */}
                  ...
                  <div className="relative z-10">
                      <div className="flex items-center gap-5 mb-8">
                        <div className="w-14 h-14 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 flex items-center justify-center text-emerald-600 dark:text-emerald-400 shadow-md dark:shadow-xl transition-all duration-300">
                            <Icon className="h-7 w-7" />
                        </div>
                        <h3 className="text-2xl font-bold text-slate-900 dark:text-slate-100 transition-colors tracking-tight">{category.title}</h3>
                      </div>
